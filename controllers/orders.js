...
export const create = async (req, res) => {
  try {
    ...
    // 檢查訂單中的商品數量是否超過上限
    const maxCartItemQuantity = 5 // 假設上限是 5 個商品
    const totalCartItemQuantity = req.user.cart.reduce((total, cartItem) => total + cartItem.quantity, 0)
    if (totalCartItemQuantity > maxCartItemQuantity) {
